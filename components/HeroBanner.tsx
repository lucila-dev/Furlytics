...
type HeroBannerProps = {
  ...
  subtitle: string;
  ctas: Cta[];
  /** When true, bleed past app main padding on small screens */
  bleed?: boolean;
};
...
export function HeroBanner({ title, subtitle, ctas, bleed = false }: HeroBannerProps) {
  return (
    <section
      className={
        bleed
          ? "relative -mx-4 h-[320px] overflow-hidden text-center sm:mx-0 sm:h-[400px] sm:rounded-[2.5rem]"
          : "relative h-[320px] overflow-hidden text-center sm:h-[420px]"
      }
    >
      <div
      ...
      <div className="relative flex h-full flex-col items-center justify-center px-5 py-6 sm:px-10">
